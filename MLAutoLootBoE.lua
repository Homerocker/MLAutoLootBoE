...
f:SetScript("OnEvent", function(self, event, arg1)
  if event == "LOOT_OPENED" then
    ...
    for i = 1, GetNumLootItems() do
      ...
      if bindType == "quest"
        ...
        elseif (itemRarity == 2 or itemRarity == 3 or (itemRarity == 4 and IsControlKeyDown() and IsAltKeyDown())) and itemLevel > 0 and MLAutoLootBoE_looters["de"] ~= nil then
          looter = MLAutoLootBoE_looters["de"]
        elseif itemID == 50274 and MLAutoLootBoE_looters["sm"] ~= nil then
          looter = MLAutoLootBoE_looters["sm"]
        elseif itemID == 45038 and MLAutoLootBoE_looters["valanyr"] ~= nil then
          looter = MLAutoLootBoE_looters["valanyr"]
        end
  ...
  elseif event == "ADDON_LOADED" and arg1 == "MLAutoLootBoE" then
    if isEnchanter and MLAutoLootBoE_looters["de"] ~= UnitName("player") then
      MLAutoLootBoE_looters["de"] = UnitName("player")
    end
  ...
  elseif event == "PARTY_LOOT_METHOD_CHANGED" then
    ...
    if masterlooterPartyID == 0 then
      MLAutoLootBoE_notify()
    end
  end
end)

function MasterLootAward(LootSlot, PlayerName)
  ...
  if (cmd == "sm" or cmd == "valanyr" or cmd == "de") then
    if name ~= "" then
      ...
      if UnitInRaid(name) then
        MLAutoLootBoE_looters[cmd] = name
        MLAutoLootBoE_notify(cmd)
      else
        print(name.." is not in raid.")
      end
    else
      MLAutoLootBoE_looters[cmd] = nil
      MLAutoLootBoE_notify(cmd)
    end
...
function MLAutoLootBoE_notify(lootType)
  if (lootType == nil or lootType == "sm") and MLAutoLootBoE_looters["sm"] ~= nil then
    local msg = itemLinks["sm"].." will be awarded to "..MLAutoLootBoE_looters["sm"].."."
  ...
  if (lootType == nil or lootType == "valanyr") and MLAutoLootBoE_looters["valanyr"] ~= nil then
    local msg = itemLinks["valanyr"].." will be awarded to "..MLAutoLootBoE_looters["valanyr"].."."
    if masterlooterPartyID == 0 and lootType ~= nil then
      SendChatMessage(msg, "RAID")
    else
      print(msg)
  ...
  if (lootType == nil or lootType == "de") and MLAutoLootBoE_looters["de"] ~= nil then
    local msg = MLAutoLootBoE_looters["de"].." is set to disenchanter."
    if masterlooterPartyID == 0 and lootType ~= nil then
      SendChatMessage(msg, "RAID")
    else
      print(msg)
